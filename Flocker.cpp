...
#include "Flocker.hh"
#include "Predator.hh" //include Predator definitions
...
{ 
	...
	max_fear_distance = max_scary_distance;
...
{
	//define fear force vector
	//get that position necessary
	//get length
	//if it's close enough
	//fear force created then afraid is true
	int i;
	glm::vec3 direction;
	glm::vec3 bat;
	double num;
	double numSqr;
	double F;
	bool afraid = false; //should the flockers be afraid?

	fear_force = glm::vec3(0, 0, 0);
	bat = position - predator_array[0]->position;
	num = glm::length2(bat);
	numSqr = num * num;

	if (numSqr <= max_squared_fear_distance) {
		F = max_squared_fear_distance / numSqr - 1.0;
		direction = (float)F * glm::normalize(bat);
