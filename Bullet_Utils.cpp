...
extern int num_predators; //Predator stuff
...
vector<glm::quat> quat_orientations; 
...
void bullet_add_obstacles()
{
  // add the ground plane (not drawn)
	//add the two slides here:

  btCollisionShape* groundShape = new btStaticPlaneShape(btVector3(0, 1, 0), 0); //ground
  btCollisionShape* leftWallShape = new btStaticPlaneShape(btVector3(1, 0, 0), 0); //need left
  btCollisionShape* rightWallShape = new btStaticPlaneShape(btVector3(-1, 0, 0), -9); //need right
  ...
  btRigidBody* leftRigidBody = new btRigidBody(leftRigidBodyCI); //follow pattern for both
  btRigidBody* rightRigidBody = new btRigidBody(rightRigidBodyCI);
 
  bullet_dynamicsWorld->addRigidBody(groundRigidBody); //follow pattern for both
...
void bullet_add_dynamic_objects()
{
  ...
  for (int i = 0; i < num_flockers; i++) {
    ...
    //use setLinearVelocity from slides
...
{  
  ...
  bullet_dynamicsWorld->setGravity(btVector3(0,-2.0f,0));
